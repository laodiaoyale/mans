...
function approveFn(intoCode) {
    ...
    obj.reviewUserName = localStorage.getItem('LoginName');
    obj.reviewUserId = localStorage.getItem('LoginId');
    obj.type = getParam.manualReviewQueue;
    ...
    $.ajax({
        ...
        url: "/api/manage/user/selectByIntoCode",//立即审批页面接口
        data: _obj,
        dataType: 'json',
        success: function (data) {
            if (data.rspCode === '000000') {
                ...
                if(_data.basicUser) {
                    ...
                    $('.match-info').attr('custCode', _data.basicUser.custCode);//将custCode绑定在内匹按钮上
                    $('.confirm-page').attr('applyTerminal',_data.basicUser.applyTerminal); // 将applyTerminal绑定在审批决策上
                }
                //查看身份证
                ...
                var ContactsList = _data.clTIntoContactsList;
                if (ContactsList) {
                    var clTIntoContactsList = $.map(ContactsList, function (o, i) {
                        return '<div class="row connent">' +
                    ...
                    $('.right-box').append(clTIntoContactsList);
                }
                var warning = _data.riskWarning;
                if (warning) {
                    ...
                    $('#risk').html('').append(riskWarn);
                }else{
                    $('.riskWarnTable').hide();
                    $('#risk').html('<li class="empt-msg" style="margin-top: 0;font-size:14px;text-align: left;padding-left: 40px;">暂无数据</li>');
                }
                var main = _data.mainframeMap;
                $("#contact option:nth-child(1)").attr("mobile",main.mobile);
                if(main){
                    $('#mainMobile').html(nullFn(main.mobile));
                    $('#mainRealName').html(nullFn(main.realName));
                    $('#mainIdCard').html(nullFn(main.idCard));//身份证号码
                }else{
                    $('#mainMobile').html('--');
                    $('#mainRealName').html('--');
                    $('#mainIdCard').html('--');//身份证号码
                }
                var calllog = _data.callLog.callLogList;
                if (calllog.length > 0) {
                    var callRecords = $.map(calllog, function (o, i) {
                        if($("#contact option:nth-child("+(2+i)+")")) {
                            $("#contact option:nth-child("+(2+i)+")").attr("mobile", calllog[i].mobile);
                        }
                        return '<div class="title-division">' +
                            '<span>联系人' + (i + 1) + '通话情况</span>' +
                            '</div>' +
                            '                            <div class="row">' +
                            '                                <div class="col-lg-4 col-md-4 col-sm-4">' +
                    ...
                    $('.operator-info').append(callRecords);
                }
                //电核操作
                if($("#contact option:selected").attr("mobile")!=undefined){
                    $("#telephone").val($("#contact option:selected").attr("mobile"));
                }
                ...
                if(records.length>0){
                    $.tmpl(TmplService.remarks,_data).appendTo("#history");
                }
...
$(function () {
    ...
    $(document).on('click','.match-info',function(){
        ...
        $('.match-info-box,.mask,.match-wraper').show();
        matchShowFn();
    })
    $(document).on('click','.match-info-close',function(){
        $('body').removeClass('ovfHiden');//使网页可滚动
        $('.match-info-box,.mask,.match-wraper').hide();
    })
    //详细信息
    $('.detail-info').click(function () {
        ...
        $('.audit-box,.mask').show();
        // $('.audit-box,.mask,.auditContainer').show();
        $('.yixin_one').click();
        $('.yixin').click();
        //汇法网
    ...
    $('.audit-close').click(function () {
        $('body').removeClass('ovfHiden');//使网页可滚动
        $('.audit-box,.mask').hide();
        // $('.audit-box,.mask,.auditContainer').hide();
    })
    //电核操作
    $("#phoneCheckSaveBtn").click(function(){
        phoneCheckSave();
    });
    ...
    $("#contact2").val($("#contact option:selected").text());
    $("#contact2").blur(function(){
        if($(this).val()!="本人"&&$(this).val()!="联系人1"&&$(this).val()!="联系人2"&&$(this).val()!="联系人3"){
            $("#contact").val("-1");
            $("#telephone").val("");
        }
    });
    ...
    // TdragFn('.auditContainer','.audit-box');
                    ...
                    if (_data != null) {
                        ...
                        if (_data.SH_QUERY_SHARE_SH != null && _data.SH_QUERY_SHARE_SH != '' && _data.SH_QUERY_SHARE_SH != undefined &&
                            ...
                            if (_data.SH_QUERY_SHARE_SH.data.Person.identification != null && _data.SH_QUERY_SHARE_SH.data.Person.identification != '' && _data.SH_QUERY_SHARE_SH.data.Person.identification != undefined) {
                                ...
                                // 性别
                                if (_identification.gender == 0) {$('.xindai_li1_con1_1').html('未知的性别');
                                } else if (_identification.gender == 1) {$('.xindai_li1_con1_1').html('男性');
                                } else if (_identification.gender == 2) {$('.xindai_li1_con1_1').html('女性');
                                } else if (_identification.gender == 9) {$('.xindai_li1_con1_1').html('未说明性别');
                                } else {$('.xindai_li1_con1_1').html('--');
                                }
                                // 出生日期
                                $('.xindai_li1_con1_2').html(nullFn(_identification.birthday));
                                // 证件类型
                                if(_identification.idType == '0'){ $('.xindai_li1_con1_3').html('身份证');
                                }else if(_identification.idType == '1'){ $('.xindai_li1_con1_3').html('户口簿');
                                }else if(_identification.idType == '2'){ $('.xindai_li1_con1_3').html('护照');
                                }else if(_identification.idType == '3'){ $('.xindai_li1_con1_3').html('军官证');
                                }else if(_identification.idType == '4'){ $('.xindai_li1_con1_3').html('士兵证');
                                }else if(_identification.idType == '5'){ $('.xindai_li1_con1_3').html('港澳居民来往内地通行证');
                                }else if(_identification.idType == '6'){ $('.xindai_li1_con1_3').html('台湾同胞来往内地通行证');
                                }else if(_identification.idType == '7'){ $('.xindai_li1_con1_3').html('临时身份证');
                                }else if(_identification.idType == '8'){ $('.xindai_li1_con1_3').html('外国人居留证');
                                }else if(_identification.idType == '9'){ $('.xindai_li1_con1_3').html('警官证');
                                }else if(_identification.idType == 'A'){ $('.xindai_li1_con1_3').html('香港身份证');
                                }else if(_identification.idType == 'B'){ $('.xindai_li1_con1_3').html('澳门身份证');
                                }else if(_identification.idType == 'X'){ $('.xindai_li1_con1_3').html('其他证件');
                                }else{ $('.xindai_li1_con1_3').html('--');
                                }
                                // 证件号码
                                $('.xindai_li1_con1_4').html(nullFn(_identification.idCard));
                                // 婚姻状态
                                if (_identification.marriage == 10) {$('.xindai_li1_con1_5').html('未婚');
                                } else if (_identification.marriage == 20) {$('.xindai_li1_con1_5').html('已婚');
                                } else if (_identification.marriage == 30) {$('.xindai_li1_con1_5').html('丧偶');
                                } else if (_identification.marriage == 40) {$('.xindai_li1_con1_5').html('离婚');
                                } else if (_identification.marriage == 90) {$('.xindai_li1_con1_5').html('未说明婚姻状况');
                                } else{$('.xindai_li1_con1_5').html('--');
                                }
                                // 最高学历
                                if (_identification.eduLevel == 10) {$('.xindai_li1_con1_6').html('研究生及以上');
                                } else if (_identification.eduLevel == 20) {$('.xindai_li1_con1_6').html('大学本科');
                                } else if (_identification.eduLevel == 30) {$('.xindai_li1_con1_6').html('大学专科和专科学校');
                                } else if (_identification.eduLevel == 40) {$('.xindai_li1_con1_6').html('中等专业学校或中等技术学校');
                                } else if (_identification.eduLevel == 50) {$('.xindai_li1_con1_6').html('技术学校');
                                } else if (_identification.eduLevel == 60) {$('.xindai_li1_con1_6').html('高中');
                                } else if (_identification.eduLevel == 70) {$('.xindai_li1_con1_6').html('初中');
                                } else if (_identification.eduLevel == 80) {$('.xindai_li1_con1_6').html('小学');
                                } else if (_identification.eduLevel == 90) {$('.xindai_li1_con1_6').html('文盲或半文盲');
                                } else if (_identification.eduLevel == 99) {$('.xindai_li1_con1_6').html('未知');
                                } else{$('.xindai_li1_con1_6').html('--');
                                }
                            } else {
                            ...
                            if (_data.SH_QUERY_SHARE_SH.data.Person.estates != null && _data.SH_QUERY_SHARE_SH.data.Person.estates != '' && _data.SH_QUERY_SHARE_SH.data.Person.estates != undefined) {
                                ...
                                var _estatesList = $.map(_estates, function (v, i) {
                                    // 居住地址
                                    if (v.address == 9) {_address = '未知';
                                    } else {_address = v.address;
                                    }
                                    // 居住情况
                                    if (v.condition == 1) { _condition = '自置（自建，自购）无贷款或贷款已还清';
                                    } else if(v.condition == 2){_condition = '按揭';
                                    } else if(v.condition == 3){_condition = '亲属楼宇';
                                    } else if(v.condition == 4){_condition = '集体宿舍';
                                    } else if(v.condition == 5){_condition = '租房';
                                    } else if(v.condition == 6){_condition = '共有住宅';
                                    } else if(v.condition == 7){_condition = '其他';
                                    } else if(v.condition == 9){_condition = '未知';
                                    }
                                    return '<tr>' +
                                        '<td>' + (i+1) + '</td>' +  // 编号
                        ...
                        if (_data.SH_QUERY_SHARE_SH != null && _data.SH_QUERY_SHARE_SH != '' && _data.SH_QUERY_SHARE_SH != undefined &&
                            ...
                            var _guaranteeInfosList = $.map(_guaranteeInfos, function (v, i) {
                                if (v) {
                                    if(v.guaranteeType == 1){ _guaranteeType = '担保中';
                                    }else if(v.guaranteeType == 2){ _guaranteeType = '担保解除';
                                    }
                                }
                                return '<tr>' +
                                    '<td>' + (i+1) + '</td>' +  // 编号
                        ...
                        if (_data.SH_QUERY_SHARE_SH != null && _data.SH_QUERY_SHARE_SH != '' && _data.SH_QUERY_SHARE_SH != undefined &&
                            ...
                            var _creditType, _accountStatus, _guaranteeWay, _repayFreq, _overduesList, _table01, indList;
                            var _creditInfosList = $.map(_creditInfos, function (v, i) {
                                var _creditDetail = v.creditDetail;
                                if(_creditDetail){
                                    // 业务类型
                                    if(_creditDetail.creditType == '1'){ _creditType = '信用贷款-现金贷（大于30天）';
                                    }else if(_creditDetail.creditType == '2'){ _creditType = '循环贷款业务';
                                    }else if(_creditDetail.creditType == '3'){ _creditType = '融资租赁业务';
                                    }else if(_creditDetail.creditType == '5'){ _creditType = '房产抵押贷款';
                                    }else if(_creditDetail.creditType == '6'){ _creditType = '车辆抵押贷款';
                                    }else if(_creditDetail.creditType == '7'){ _creditType = '信用贷款-短期现金贷(<=30天)';
                                    }else if(_creditDetail.creditType == '8'){ _creditType = '信用贷款-消费分期贷（有业务场景）';
                                    }else if(_creditDetail.creditType == 'Z' || _creditDetail.creditType == 'z'){ _creditType = '其他业务';
                                    }
                                    // 合同状态
                                    if(_creditDetail.accountStatus == 0) { _accountStatus = '正常未结清';
                                    }else if(_creditDetail.accountStatus == 1) { _accountStatus = '代偿未结清';
                                    }else if(_creditDetail.accountStatus == 2) { _accountStatus = '正常结清';
                                    }else if(_creditDetail.accountStatus == 3) { _accountStatus = '代偿结清';
                                    }else if(_creditDetail.accountStatus == 4) { _accountStatus = '呆账';
                                    }else if(_creditDetail.accountStatus == 5) { _accountStatus = '核销';
                                    }else if(_creditDetail.accountStatus == 6) { _accountStatus = '以资抵债';
                                    }else if(_creditDetail.accountStatus == 7) { _accountStatus = '提前还款未结清';
                                    }else if(_creditDetail.accountStatus == 8) { _accountStatus = '提前还款结清';
                                    }
                                    // 担保方式
                                    if(_creditDetail.guaranteeWay == 1){ _guaranteeWay = '质押（含保证金）';
                                    }else if(_creditDetail.guaranteeWay == 2){ _guaranteeWay = '抵押';
                                    }else if(_creditDetail.guaranteeWay == 3){ _guaranteeWay = '自然人保证';
                                    }else if(_creditDetail.guaranteeWay == 4){ _guaranteeWay = '信用/免担保';
                                    }else if(_creditDetail.guaranteeWay == 5){ _guaranteeWay = '组合（含自然人保证）';
                                    }else if(_creditDetail.guaranteeWay == 6){ _guaranteeWay = '组合（不含自然人保证）';
                                    }else if(_creditDetail.guaranteeWay == 7){ _guaranteeWay = '农户联保';
                                    }else if(_creditDetail.guaranteeWay == 9){ _guaranteeWay = '其他';
                                    }
                                    // 还款频率
                                    if(_creditDetail.repayFreq == '01'){ _repayFreq = '日';
                                    }else if(_creditDetail.repayFreq == '02'){ _repayFreq = '周';
                                    }else if(_creditDetail.repayFreq == '03'){ _repayFreq = '月';
                                    }else if(_creditDetail.repayFreq == '04'){ _repayFreq = '季';
                                    }else if(_creditDetail.repayFreq == '05'){ _repayFreq = '半年';
                                    }else if(_creditDetail.repayFreq == '06'){ _repayFreq = '年';
                                    }else if(_creditDetail.repayFreq == '07'){ _repayFreq = '一次性';
                                    }else if(_creditDetail.repayFreq == '08'){ _repayFreq = '不定期';
                                    }else if(_creditDetail.repayFreq == '99'){ _repayFreq = '其他';
                                    }
                        ...
                        if (_data.SH_QUERY_SHARE_SH != null && _data.SH_QUERY_SHARE_SH != '' && _data.SH_QUERY_SHARE_SH != undefined &&
                            ...
                            var _SpecTradeList = $.map(_SpecTrade, function (o, i) {
                                var _i;
                                if (i == 'extends') {_i = '展期'};
                                if (i == 'angents') {_i = '担保人代还'};
                                if (i == 'leases') {_i = '以资抵债'};
                                if (i == 'over30') {_i = '逾期1-29天'};
                                if (i == 'over60') {_i = '逾期30-59天'};
                                if (i == 'over90') {_i = '逾期60-89天'};
                                if (i == 'overL') {_i = '长期拖欠(90天以上)'};
                                if (i == 'illegal') {_i = '法律诉讼(已判决生效)'};
                                if (i == 'cheat') {_i = '诈骗'};
                                if (i == 'advance') {_i = '提前还款'};
                                if (i == 'other') {_i = '其他'};
                                if (o) {
    ...
    $(document).on('click', '.renhang', function () {
        // $('.pboc-box,.renhang-con-hock').show();
        PBOCReportFn();
    })
    ...
    //魔蝎
    $(document).on('click','.scorpion',function(){
        moxieFn();
    })
    function moxieFn(){
        var obj = {
                ...
                if (data.rspCode === '000000') {
                    ...
                    if (_data.type == '' || _data.type == null || _data.type == undefined) {
                        ...
                        if (TBList) {
                            ...
                            //         '<td>'+ (o.isDelete == 1?'是':'否') +'</td>' +
                            //         '</tr>';
                            // }).join('');
                            // $('.TBCreditList').html('').append(TBListCon);
                        } else {
                            $('.Taobao-credit-box .table,#Taobao-credit-page').hide();
                        ...
                        if (TB13List) {
                            ...
                            //         '<td class="JustifyLeft">'+ nullFn(o.name) +'</td>' +
                            ...
                            $('.TaobaoTB13-credit-box .table,#TB13-credit-page').hide();
    ...
    $('.heiList-close').click(function () {
        $('body').removeClass('ovfHiden');//使网页可滚动
        $('.mask,.heiList-box,.blackContainer,.pboc-box,.Taobao-credit-box,.TaobaoTB13-credit-box').hide();
    })
    ...
    $("#com-textarea").on('input', function () {
        if ($('#com-textarea').val().length >= 500) {
            showMsg($('.error-msg'), '最多不得超过500个字符');
        }
    });
    var _name = localStorage.getItem('LoginName');
    $('._approver').html(_name);//审批人
    //提交&取消 提交审批决策
    $(document).on('click', '.submitB', function () {
        $('.confirm-page,.mask').show();
    })
    //点击确认页确认
    $(document).on('click', '.confirm-sure', function () {
        var _applyTerminal = $('.confirm-page').attr('applyTerminal');
        var obj = new Object();
        obj.mobile = $('#mobile').html();
        obj.name = $('#name').html();
        obj.reviewStatus = reviewStatusFn($('.radio-span.active').next().html());//审核状态
        obj.intoCode = getParam.intoCode;
        var selects = reviewResultCodeFn(obj.reviewStatus).split('-');//核批代码
        obj.reviewResultCode = selects[0];//核批代码（拒绝、拒绝并加入黑名单）
        obj.review_user = $('._approver').html();//审批人
        obj.blacklist_type = blackType(reviewStatusFn($('.radio-span.active').next().html()));//黑名单类型
        obj.remark = $("#textareaContents").val();
        if(obj.reviewStatus === 1){
            if($('#province').find("option:selected").text()=='请选择'||$('#city').find("option:selected").text()=='请选择'||$('#area').find("option:selected").text()=='请选择'){
                showMsg($('.error-msg'), '请选择完整信息');
                return ;
            }
        }
        if(obj.reviewStatus === 2 ){
            if($('#province1').find("option:selected").text()=='请选择'||$('#city1').find("option:selected").text()=='请选择'||$('#area1').find("option:selected").text()=='请选择'||$('#blacklist_type').find("option:selected").text()=='请选择'){
                showMsg($('.error-msg'), '请选择完整信息');
                return ;
            }
        }
        if(obj.reviewStatus === 1 || obj.reviewStatus === 2 ){
            if(obj.reviewResultCode == ''){
                showMsg($('.error-msg'), '原因子类2不得为空');
                return ;
            }
        }else if(obj.remark == ''){
            showMsg($('.error-msg'), '备注不得为空');
            return ;
        }else
        obj.reviewUserId = localStorage.getItem('LoginId');
        obj.manual_review_queue = getParam.manualReviewQueue;
        obj.refuseCodeIdLevel3 = parseInt(selects[2])!== NaN ?selects[2] :selects[3];   //原因子类2 ID
        obj.type = getParam.manualReviewQueue;
        obj.applyTerminal = _applyTerminal;
        var _obj = JSON.stringify(obj, 'utf-8');
        $.ajax({
            headers: {
                token: localStorage.getItem('LoginToken')
            ...
            type: 'POST',
            contentType: "text/html; charset=UTF-8",
            url: "/api/manage/user/insertIntoInfoApproveMessage",//审批队列进件决策接口
            data: _obj,
            dataType: 'json',
            success: function (data) {
                if (data.rspCode === '000000') {
                    ...
                    setTimeout(function () {
                        if (obj.manual_review_queue == 1) {
                            window.location.href = 'approvalQueueOne.html'
                        } else if (obj.manual_review_queue == 2) {
                            window.location.href = 'approvalQueueTwo.html'
                        } else if (obj.manual_review_queue == 3) {
                            window.location.href = 'approvalQueueThree.html'
                        }
                    })
                } else if (data.rspCode === '400004') {
                    showMsg($('.error-msg'), data.rspMsg);
                    setTimeout(function () {
                        if (obj.manual_review_queue == 1) {
                            window.location.href = 'approvalQueueOne.html'
    ...
    $(document).on('click', '.confirm-cancel', function () {
        $('.confirm-page,.mask').hide();
    })
    //点击撤销
    $(document).on('click','.undo',function(){
        $('.mask,.undo-box').show();//撤销原因显示
        causeRevocation();//渲染撤销原因
    })
    $(document).on('click','.undo-close,.undo-cancel',function(){
        $('.mask,.undo-box').hide();
    })
    $(document).on('click','.undo-sure',function(){
        $('.undo-box,.undo-success').hide();
        $('.undoSureBox,.undo-sure-body').show();//确认撤销弹框显示
    })
    $(document).on('click','.undoS-yes',function(){
        $('.undo-sure-body').hide();
        orderCancellation();//调取订单撤销接口
    })
    $(document).on('click','.undoS-no',function(){
        $('.undo-success,.undoSureBox,.undo-box,.mask').hide();//撤销取消
    })
    $(document).on('click','.undoSureBoxClose',function(){
        $('.undoSureBox,.mask').hide();//撤销完成关闭弹框
    })
    //点击返回
    $(document).on('click', '.escB', function () {
        var _escBmanualReviewQueue = getParam.manualReviewQueue;
        setTimeout(function () {
            if (_escBmanualReviewQueue == 1) {
                window.location.href = 'approvalQueueOne.html'
            } else if (_escBmanualReviewQueue == 2) {
                window.location.href = 'approvalQueueTwo.html'
            } else if (_escBmanualReviewQueue == 3) {
                window.location.href = 'approvalQueueThree.html'
            }
        }, 100)
    })
    //拒绝原因循环
    //获取一级拒绝原因
    $(document).on('click','#rejT',function(){
        getFirstRejectReasonFn($('#province'),0);
    })
    $(document).on('click','#blacklistT',function(){
        getFirstRejectReasonFn($('#province1'),0);
    })
    //获取原因子类1
    $('#province').change(function () {
        var level = $('#province').find("option:selected").val(),
            _parentId = parseInt(level);
        getFirstRejectReasonFn($('#city'),_parentId);
    })
    $('#province1').change(function () {
        var level = $('#province1').find("option:selected").val(),
            _parentId = parseInt(level);
        getFirstRejectReasonFn($('#city1'),_parentId);
    })
    //获取原因子类2
    $('#city').change(function () {
        var level = $('#city').find("option:selected").val(),
            _parentId = parseInt(level);
        getFirstRejectReasonFn($('#area'),_parentId);
    })
    $('#city1').change(function () {
        var level = $('#city1').find("option:selected").val(),
            _parentId = parseInt(level);
        getFirstRejectReasonFn($('#area1'),_parentId);
    })
    // 一级原因改变二级原因更新
    $('#province').change(function(){
        $('#city').empty();
    ...
    $('#passT').click(function () {
        $('#pass,.sp-msg').removeClass('hidden');
        $('#rej,#blacklist').addClass('hidden');
        $('.spR').hide();
    })
    $('#rejT').click(function () {
        $('#rej,.sp-msg').removeClass('hidden');
        $('#pass,#blacklist').addClass('hidden');
        $('.spR').show();
    })
    $('#blacklistT').click(function () {
        $('#blacklist,.sp-msg').removeClass('hidden');
        $('#pass,#rej').addClass('hidden');
        $('.spR').show();
    })
    ...
    $(document).on('click','.clickToDetail',function () {
        var Height = $(document).scrollTop();
        getTbDetalTemplate($(this));//渲染数据
        $('.tbOrderDetail-box').css({'top': Height + 150 + 'px'});
        $('.tbOrderDetail-box,.tbMask').show();
    });
    $('.tbDetail-close').click(function () {
        $('.tbOrderDetail-box,.tbMask').hide();
    });
})

//获取一级拒绝原因
function getFirstRejectReasonFn(element,parentId) {
    if(isNaN(parentId)){
        showMsg($('.error-msg'), '请选择完整信息');
        return;
    }
    if(parentId == 0){
        var _reviewStatus = reviewStatusFn($('.radio-span.active').next().html());
        if(_reviewStatus === 1){
            if($('.province').find("option:selected").text() == '请选择'){
                showMsg($('.error-msg'), '请选择完整信息');
                return ;
            }
        }else if(_reviewStatus === 2){
            if($('.province1').find("option:selected").text() == '请选择'){
                showMsg($('.error-msg'), '请选择完整信息');
                return ;
            }
        }
    ...
    $.ajax({
        ...
        success : function (data) {
            if(data.rspCode==='000000'){
                ...
                var firstReasonList = $.map(dataList,function (o,i) {
                    return '<option data-days="'+o.refuse_days+'" value="'+o.id+'" id="'+o.id+'">' + o.refuse_code +'-'+o.refuse_des + '</option>';
                }).join('');
                element.html('<option value="">请选择</option>').append(firstReasonList);
            }else if(data.rspCode==='-999999'){
                localStorage.removeItem("LoginName");
                localStorage.removeItem("LoginToken");
                localStorage.removeItem("userNo");
...
//渲染内匹弹框数据
function matchShowFn(){
    var _custCode = $('.match-info').attr('custCode');
    var obj = {
        'custCode':_custCode
        // 'custCode':'220180420170824816939'
        // 'custCode':'J20171210115115882021'
    };
    var _obj = JSON.stringify(obj,'utf-8');
    $.ajax({
        headers: {
        ...
        type: "POST",
        contentType: "text/html; charset=UTF-8",
        url: "/api/manage/user/internalMatch",//内匹
        data: _obj,
        dataType: 'json',
        success : function (data) {
            if(data.rspCode==='000000'){
                if(data.body.rspCode=='0000'){
                    $('.match-name').html($('#name').html());
                    $('.match-idCard').html($('#idcard').html());
                    var _resultList = data.body.resultList;
                    if(_resultList.length>0){
                        var _matchList = $.map(_resultList,function(o,i){
                            return '<li><a href="approvalHistoryMessage.html?intoCode='+o.intoCode+'&status='+o.status+'&isJump=jump" target="_blank">历史'+ (i+1)+'</a></li>';
                        }).join('');
                        $('.match-list').html('').append(_matchList);
                    }else{
                        $('.match-list,.match-person').hide();
                        $('.no-info').show();
                    }
                }else{
                    showMsg($('.error-msg'), data.body.rspMsg);
                }
            }else if(data.rspCode==='-999999'){
                localStorage.removeItem("LoginName");
                localStorage.removeItem("LoginToken");
...
function reviewResultCodeFn(str) {
    if (str == 0) {
        return $('#through').find("option:selected").text();
    } else {
        if (str == 1) {
            return $('#area').find("option:selected").text() +'-'+$('#area').find("option:selected").val();
        } else if (str == 2) {
            return $('#area1').find("option:selected").text()+'-'+$('#area').find("option:selected").val();
        }
    }
}

function blackType(str) {
    if (str == 2) {
        var item = $('#blacklist_type').find('option:selected').val();
        if (item >= 0) {
            return item;
        } else {
            return '';
...
function causeRevocation(){
    ...
    $.ajax({
        ...
        type: "POST",
        contentType: "text/html; charset=UTF-8",
        url: "/api/sys/dataDictionary/v1/",//运营后台撤销原因
        data: _obj,
        dataType: 'json',
        success :function(data){
            if (data.rspCode === '000000') {
                var _cancelStatusList = data.body.cancelLoanStatusList;
                if(_cancelStatusList){
                    var _cancelList = $.map(_cancelStatusList,function(o,i){
                        return '<label class="radio-inline">' +
                            '          <input type="radio" name="inlineRadioOptions" value="'+ o.statusid +'"> '+ o.statusmesssage +
                            '    </label>';
                    }).join('');
                    $('.check-box').html('').append(_cancelList);
                }
            }else if (data.rspCode === '-999999') {
                localStorage.removeItem("LoginName");
                localStorage.removeItem("LoginToken");
...
function orderCancellation(){
    var _causeCode = $('.radio-inline input[name="inlineRadioOptions"]:checked ').val();
    var _reviewUser = localStorage.getItem('LoginName');
    var obj = {
        'orderNo' : $('#intoCode').html(),//进件编号
        // 'orderNo' : '120171205223119384025',//进件编号
        'causeCode' : _causeCode, //选中的撤销原因，
        'reviewUser': _reviewUser,
        'type' : getParam.manualReviewQueue
    };
    var _obj = JSON.stringify(obj,'utf-8');
    $.ajax({
        headers: {
        ...
        type: "POST",
        contentType: "text/html; charset=UTF-8",
        url: "/api/manage/user/cancelLoan/v1",//运营后台订单撤销
        data: _obj,
        dataType: 'json',
        success : function(data){
            if(data.rspCode === '000000'){
                $('.undo-success').show();//显示撤销成功
                $('.undoSureBoxClose').click(function(){
                    window.history.go(-1);
                })
                setTimeout(function(){
                    window.history.go(-1);
                ...
                $('.undoSureBox,.mask').hide();
                showMsg('.error-msg', data.rspMsg);
            }
        }
    })
}
function phoneCheckSave() {
    var _row = $("#phoneCheckSaveBtn").parent(".row");
    var contact2 =  $.trim($("#contact2",_row).val());
    var telephone = $.trim($("#telephone",_row).val());
    var answering = $("#answering option:selected",_row).text();
    var answeringState = $("#answering option:selected",_row).val();
    var remark = $.trim($("#remark").val());
    if(contact2.length==0){
        showMsg('.error-msg', "请选择联系人");
        return false;
    }
    if(contact2.length>10){
        showMsg('.error-msg', "请输入10字以内的联系人");
        return false;
    }
    if(telephone.length==0){
        showMsg('.error-msg', "请输入电话");
        return false;
    }
    if(telephone.length>15){
        showMsg('.error-msg', "请输入15数以内的电话");
        return false;
    }
    // if(remark.length==0){
    //     showMsg('.error-msg', "请输入备注");
    //     return false;
    // }
    if(remark.length>500){
        showMsg('.error-msg', "请输入500以内的备注");
        return false;
    }
    var obj = {
        "intoCode":getParam.intoCode,
        "reviewUser":localStorage.getItem('LoginId'),
        "contractName":contact2,
        "telephone":telephone,
    ...
    $.ajax({
        ...
        type: "POST",
        contentType: "text/html; charset=UTF-8",
        url: "/api/manage/phoneCheck/save",//联系人通话记录明细接口
        data: _obj,
        dataType: 'json',
        success: function (data) {
            if (data.rspCode === '000000') {
                ...
                var _li = $("<li>");
                var _div = $("<div>").html(contact2);
                var _div2 = $("<div>").html('<label>电话</label>' + telephone);
                var _div3 = $("<div>").html('<label>接听状态</label>' + answering);
                var _span = $("<span>").text(remark).addClass("remark");
                var _div4 = $("<div>").html('<label>备注</label>').append(_span);
                var _div5 = $("<div>").html('提交时间 ' + getNowFormatDate());
                $("#phoneCheckOperationList").append(_li.append(_div).append(_div2).append(_div3).append(_div4).append(_div5));
                $("#contact",_row).val("0");
                $("#contact2").val($("#contact option:selected").text());
                if($("#contact option:selected").attr("mobile")!=undefined){
...
var tbDetailTitleVk = {"address":"交易地址详情","seller":"淘宝卖家详情","logistics":"物流详情","subOrders":"子订单详情","buyer":"淘宝买家详情"};
