$(function () {
    ...
    if(window.location.href.indexOf("resource_add.html")>-1){
    ...
    if(window.location.href.indexOf("resource_add.html?type=edit")>-1){
        $("#path").html(" > 团队修改");
        initUser();
        //点击其他地方收起下拉框
        $("body").on("click",function(event){
    ...
    $("#userTable").on('click','.infoTlt',function () {
        var resourceInfo =  JSON.stringify($(this).parents("tr").data());
        sessionStorage.setItem("resourceInfo",resourceInfo);
        window.location.href = "resource_info.html";
    });
    $("#userTable").on('click','.redactTlt',function () {
        var resourceInfo =  JSON.stringify($(this).parents("tr").data());
        sessionStorage.setItem("resourceInfo",resourceInfo);
        window.location.href = "resource_add.html?type=edit";
    });
...
function initUser(){
    var resourceInfo = JSON.parse(sessionStorage.getItem("resourceInfo"));
    $("#id").val(resourceInfo.id);
    $("#name").val(resourceInfo.name);
    $("#distribution").val(resourceInfo.distribution);
    $("#mobile1").val(resourceInfo.mobile1);
    $("#mobile2").val(resourceInfo.mobile2);
    $("#idCard").val(resourceInfo.idCard);
    $('#address').val(resourceInfo.address);
    $("#bankCard").val(resourceInfo.bankCard);
    $("#bankName").val(resourceInfo.bankName);
    $("#bankAddress").val(resourceInfo.bankAddress);
    $("#company").val(resourceInfo.company);
    $("#region").val(resourceInfo.region);
}
